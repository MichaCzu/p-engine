...
struct View {
	...
	pge::Pixel get_pos() { return pge::Pixel{ x, y }; }
	pge::Pixel set_pos(pge::Pixel* newpos) { x = newpos->x; y = newpos->y; }

	pge::Pixel size() { return pge::Pixel{ w, h }; };
};
...
bool contains(int32_t _x, int32_t _y, uint32_t _w, uint32_t _h, View* _vp = NULL);
	...
	namespace window {
		...
		void create(int width = DEFAULT_WIN_WIDTH,
			...
			uint32_t window_flags = SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_MOUSE_CAPTURE,
