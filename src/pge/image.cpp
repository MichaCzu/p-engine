...
namespace pge {
	namespace image {
			...
			// keying - 0xRRGGBBAA
		{
			...
			if (!loadSurface) {
				if (pge::is_running())
					debug::prompt("Missing file", "Couldn't load" + _path + "\nreason: " + IMG_GetError(), pge::debug::critical);
				debug::log("Aborting...");
				...
				GPU_SetSnapMode(vTexture[_id]->text, GPU_SNAP_POSITION);
		...
		bool drawExt(uint16_t _id, int32_t _x, int32_t _y, const Rect* _clip,
			...
			GPU_FlipEnum _flip)
		{
			return draw(_id, _x - _center->x*_xscale, _y - _center->y * _yscale, _clip, _center, _angle, _xscale, _yscale, _flip);
		}
